...
/* The table, written out rather than built on first use.
 *
 * IT USED TO BE BUILT LAZILY, and the argument for that was that 256
 * numbers in a source file are 256 numbers nobody can check by reading.
 * That argument was answered on the pull request and it was wrong twice
 * over.
 *
 * A lazily built global is a data race the moment two blob caches in one
 * process checksum at the same time: the store serialises its own
 * callers, and nothing serialises this. Identical values do not make
 * concurrent non-atomic accesses defined. And it is worse than
 * pedantry, because the sentinel was entry 1 while the builder filled 0
 * to 255 in order — so a second thread could find entry 1 already
 * written, conclude the table was ready, and read an entry the writer
 * had not reached. That is a *wrong checksum*, which is the one outcome
