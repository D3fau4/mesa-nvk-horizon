...
#
# and of nothing else. Editing a comment in a script, a document or a
# test does not change it, which is the intended behaviour: none of those
# can change a shader binary.
#
...
# is what this port claims to be built against.
{
    ...
    find horizon -type f \( -name '*.c' -o -name '*.h' \) |
        LC_ALL=C sort |
        while IFS= read -r f; do
            printf '%s  %s\n' "$(sha256sum "$f" | cut -d' ' -f1)" "$f"
        done
