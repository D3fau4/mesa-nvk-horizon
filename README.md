...
164 console logs are kept in [`docs/hw-logs/`](docs/hw-logs/README.md), verbatim,
...
- **Never verified on hardware:** display mode changes, docked resolution,
  `VK_SUBOPTIMAL_KHR`, `VK_PRESENT_MODE_IMMEDIATE_KHR` through Vulkan, and two surfaces
  over two `NWindow`s. Patches `0068`, `0072` and `0073` are cross-built only.
...
| `mesa-patches/` | Our entire delta against Mesa: 73 tracked patches — platform support, `nvkmd_horizon`, WSI |
