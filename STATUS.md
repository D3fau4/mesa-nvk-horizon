...
## Mesa 26.1.8, a clean miss — and 26.2.1 exists, so D21 is live (2026-08-22)

Two separate things happened upstream, and they are separate on purpose: one is an
update and one is a decision.

### 1. The pin moved to `mesa-26.1.8`, and it costs nothing

`mesa-26.1.8` was tagged on 2026-08-19, above the pinned `mesa-26.1.7`. It is a point
release inside the series D2 chose —
[notes](https://docs.mesa3d.org/relnotes/26.1.8.html): "New features: None", 8 bug
fixes, 98 files changed, +5012/-358.
...
is the one the file-level diff actually supports.

Two things in the diff look like they might touch this port and do not, so they were
read rather than waved past:

| what it looks like | what it is |
|---|---|
| two `src/vulkan/wsi/` files changed | `wsi_common_{metal,wayland}.c`. Neither is a file we patch; the Horizon WSI backend is a patch of ours, not an upstream file. The four *upstream* `src/vulkan/wsi/` files `mesa-patches/` does write to — `wsi_common.{c,h}`, `wsi_common_private.h`, `meson.build` — are untouched; the fifth, `wsi_horizon.c`, is ours and exists only because a patch creates it |
| 29 `subprojects/*-rs.wrap` changed | `source_url` edits only — `crates.io/api/v1/crates/…/download` → `static.crates.io/crates/…/download`. Checked line by line: 29 `-source_url` and 29 `+source_url` and **no other changed line in any wrap**, so no crate version and no `source_hash` moves |

That second one is the only part of this release with a route to our build at all, because
`scripts/fetch-mesa-subprojects.sh` runs `meson subprojects download`, which reads those
URLs. It is a non-event and better than that: `static.crates.io` is the host
`scripts/fetch-rust-crates.sh` already uses, i.e. the one already proven reachable through
this environment's proxy. The build confirms it rather than assuming it —
`fetch-mesa-subprojects: downloading 29 wrap(s)` followed by 29 `-> done`, every one from
`https://static.crates.io/`.

`meson.build` itself is byte-identical between the two tags, so the Meson floor
(`>= 1.4.0`, `meson.build:9`) and the bindgen/cbindgen floors recorded in
`toolchain/versions.env` all still describe the tree.
...
gap is narrower than it sounds, because the 26.1.7 → 26.1.8 diff cannot change generated
code for GM20B at all.

### 2. `mesa-26.2.1` exists — D21's gate has opened, and it is the owner's call

This is the part that needs a person. `docs/known-risks.md` R14 and D21 both recorded the
26.2 move as deferred *for a specific reason*: 26.2.0's own release notes tell
stability-minded users to wait for 26.2.1, and on 2026-08-12 that release did not exist.

**It does now.** `mesa-26.2.1` was tagged 2026-08-20 @
`da14d65e4499e66468094be52bff9ea0915a695e`
([notes](https://docs.mesa3d.org/relnotes/26.2.1.html)) — "New features: None", 19 bug
fixes, and its notes carry no stick-with-a-previous-release advice. So the deferral
reason is spent; what is left is the cost, and only the owner can weigh that.

What it offers this port, from 26.2.1 itself:

- an **NVK/Kepler NAK register-allocation panic** fixed — `Failed to find free register`
  in `assign_regs.rs`, reported against a `wgpu` application. Kepler-reported, but
  `assign_regs.rs` is not shader-model-gated the way an encoder is, so it is worth
  reading properly before dismissing it as somebody else's hardware;
- the two NAK fixes this port **already has** by way of 26.1.7 (`Serialize carry access
  in instr_sched_prepass`, `Do not encode a RZ for src2 on FMNMX`) — so on those two,
  26.2.1 offers nothing 26.1.8 does not already carry.

Plus what the 26.2 *series* offers, unchanged from the D21 row: `VK_EXT_mesh_shader` on
NVK, `VK_KHR_shader_fma`, `VK_EXT_shader_atomic_float`,
`VK_NV_shader_atomic_float16_vector`, `VK_NVX_binary_import`, and `VK_GOOGLE_display_timing`
— which is WSI-shaped and would meet our backend directly.

What it costs, re-measured against the pin as it now stands rather than quoted from the
26.1.6-era numbers:
...
to the owner with its gate now open.**
