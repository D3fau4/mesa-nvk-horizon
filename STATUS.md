...
**Last updated:** 2026-08-31 (the pin moved `mesa-26.1.7` → `mesa-26.1.8`, a point release inside the series D2 chose. It is a **clean miss**: none of the 117 Mesa files `mesa-patches/` modifies is among the 98 it changes, there is no `src/nouveau/` commit in it at all, and the series' net diff is *byte-identical* on the two bases — `sha256 e5a6593f…`, 13021 lines, both. 49 of 49 apply, no fuzz, no rejects. **Separately, and not taken here: `mesa-26.2.1` now exists**, which is the release 26.2.0's own notes told stability users to wait for, so the reason D21 could not be decided has gone. The cost has not: 55 of the 117 files we patch move under it. That is the owner's call)
**Branch:** `claude/bold-goodall-xopm1d`
...
| | |
|---|---|
| **Phase** | **PHASE 7 IS COMPLETE (run 31, 2026-08-11, `PASS 45/45`).** The shader disk cache is enabled, correct and measured on a Nintendo Switch: **`vkCreateComputePipelines` 5342 us cold against 442 us warm — 91% of the compile, 12x faster** — on the driver's own cache path with no environment override and no `VkPipelineCache` passed, so nothing but the disk could have answered it. The shader that came back is right in full (`4096/4096 words match`, poisoned tail untouched) and NVK compiled nothing (`hits = 2, misses = 0`). The cold baseline is self-certified: the test records one only when the driver reported `0 entries` at startup, and runs 29 and 30 both wrote nothing rather than record a warm create as cold. Before it: **PHASE 6 IS COMPLETE, layout included.** The operator confirms the pattern renders correctly on the console — the one thing no measurement in this phase could produce, because every number here is about frames arriving and none about what was inside them. Run 16 is the run in which every piece of coverage this branch built actually executed. `t_vk_swapchain` **PASS 125/125** and `t_nwindow` **PASS 119/119** on one build. Three images present 90 of 90 at a mean of exactly **16666 us with 89 of 89 intervals inside 10%**; the infinite-timeout session ran for the first time at **20 of 20, 19 of 19 within 10%**; the copy fallback presented the pattern for the first time; both `VK_TIMEOUT` and `VK_NOT_READY` were produced and asserted. No MMU fault, no hang. A `VK_KHR_swapchain` presents on a Nintendo Switch through NVK over Horizon's VI compositor, zero-copy, at 89 of 89 intervals inside 10% of a 60 Hz refresh |
...
| **Open decisions** | **D7 and D21.** **D21 (raised 2026-08-10): whether to move off the 26.1 series onto Mesa 26.2.** Still not taken, but what changed on 2026-08-31 is that it *can* now be: **`mesa-26.2.1` exists** (released 2026-08-20), the release 26.2.0's own notes told stability users to wait for. The cost is unchanged and was re-measured against the current pin — **3569 files changed `mesa-26.1.8` → `mesa-26.2.1`, 55 of the 117 `mesa-patches/` modifies among them** (21 NAK, 8 `src/nouveau/vulkan/`, 4 `src/vulkan/wsi/`, both `vk_image.{c,h}`, 9 Rust helper crates) — so it is a rebase branch with its own console run, not a `versions.env` edit, and it is the owner's call. The *point* releases inside the pinned series were taken: **the pin is `mesa-26.1.8`** (2026-08-31), series applying 49 of 49 with a byte-identical delta on the old and new base. D18 (`minImageCount`) closed: it stays **2** — two images present 90 of 90; the compositor was never the limit, the retry loop was. D19 closed by run 13: **`async=false` is deleted** (patch 0067), because `async=true` plus the sleep presented 90 of 90 on two images without it. **D20** — the untracked Mesa commit, which this row previously called D18 as well — closed 2026-08-09 by exporting it as patch **0071**; see the collision note beneath the decisions table |
...
## The pin moved to `mesa-26.1.8`, and 26.2.1 exists now (2026-08-31)

**Cross build only.** No console has run any of this — and none has run 26.1.7
either, so the newest hardware evidence in this file is still run 31 on a tree
two point releases behind the pin.

Two things came out of checking upstream, and they are not the same kind of
thing. One is an update and was taken. The other is a decision and was not.

### 1. `mesa-26.1.7` → `mesa-26.1.8`, taken

`0fadfea4f394211946f308458f614839ef253ee8`, tag object `d6393a37abc5`, released
2026-08-19, [release notes](https://docs.mesa3d.org/relnotes/26.1.8.html).
Inside the series D2 chose, so this is R14 housekeeping and not a reopening of
anything. "New features: None", 76 commits over 98 files.

**It is a clean miss, and this one is measured rather than asserted.** The
series modifies 117 files that exist in Mesa and adds 14 of its own; **none of
the 117 is among the 98** the release touches. There is **no `src/nouveau/`
commit in the range at all** — the first point release since the series began of
which that is true, so NVK, NAK, NIL and `nvkmd` are untouched.

The strong form of the same claim, because a clean file-level miss still leaves
`git am` free to have done something different:

```sh
$ git -C mesa diff mesa-26.1.7^{} <49 patches applied> | sha256sum
e5a6593f60eeb96b4bbf03c473c0dbe3a3f6e86c691fa014aafb54e703c005a0   13021 lines
$ git -C mesa diff mesa-26.1.8^{} <49 patches applied> | sha256sum
e5a6593f60eeb96b4bbf03c473c0dbe3a3f6e86c691fa014aafb54e703c005a0   13021 lines
```

**Byte-identical.** The bump changed nothing whatsoever about our delta. The old
side was produced by applying the same 49 files into a throwaway worktree at
`mesa-26.1.7^{}`; `scripts/apply-mesa-patches.sh` reports `49 patches` on the
new base, no fuzz, no rejects.

What can still reach a Switch shader is the shared middle-end, and in this
release it is four correctness fixes rather than any feature:

| file | what 26.1.8 does to it | reaches a Switch? |
|---|---|---|
| `nir/nir_opt_peephole_select.c` | do not select on `read_invocation` | **yes** — a common NIR pass, NVK runs it |
| `nir/nir_search.c` | reset FP state between commutative match attempts | **yes** — the algebraic optimiser |
| `nir/nir_gather_info.c` | set `fs.color_is_dual_source` for `DUAL_SRC_BLEND` locations | only via a dual-source-blend FS; nothing here has one |
| `spirv/vtn_variables.c` (+ the `glsl` and `mesa/shader_query` halves) | mark `Location`-decorated variables `explicit_location` | **yes** — SPIR-V is how every shader enters NVK |

One build-side change is worth naming because this project reached the same
conclusion independently and painfully: **"subprojects: Use CDN URLs for
crates.io"** rewrites every `subprojects/*-rs.wrap` `source_url` from
`crates.io/api/v1` to `static.crates.io`, same `source_filename`, same version,
same `source_hash`. `scripts/fetch-rust-tools.sh:99` and
`scripts/fetch-rust-crates.sh:220` already fetch from `static.crates.io`. It is
observed working, not assumed: the run below shows
`fetch-mesa-subprojects: downloading 29 wrap(s)` and every one of the 29 named
...
| command | result |
|---|---|
| `scripts/fetch-mesa.sh` | `checked out mesa-26.1.8, verified HEAD = 0fadfea4…`, 508 MB |
| `scripts/apply-mesa-patches.sh` | `mesa at 098bb28 — 0fadfea4… + 49 patches` |
| `scripts/run-host-tests.sh` | **PASS 305/305** over 7 suites (`h_align` 20, `h_va_space` 21, `h_syncpt_math` 30, `h_cmds` 39, `h_status` 16, `h_log` 8, `h_blob_cache` 171), ASan+UBSan |
| `check-{layering,no-abs-paths,mesa-test-parity,history-intact,rust-target,dispatch-complete}.sh` | all OK |
| `scripts/ci-build-archives.sh` | **exit 0** — `OK — every archive built, 37 .nro link them`, `check-dispatch-complete: OK (825 entry points named, 234 core, 1 allowed absence)`, `check-tls-relocs: OK (3 object(s) use TLS, all with relocations, 350 scanned)` |

The host tests say nothing about the bump — they are Mesa-free by design
(`CLAUDE.md` § Layer rules) — and are recorded because a green baseline that was
not run is not a baseline. The claim the bump rests on is
`ci-build-archives.sh`: it compiled Mesa's core, NAK/NIL's Rust half and the
whole nouveau Vulkan driver against the new pin with the series applied, and
linked all 37 `.nro`. That is a **cross build**. It says nothing about a
console, and `ci-build-archives.sh` says so itself in its last line.

Non-finding, recorded so it is not re-investigated: the log carries 358
`error: 'rustfmt' is not installed` lines from bindgen. The next line each time
is `Failed to run rustfmt: Internal rustfmt error (non-fatal, continuing)` —
bindgen declining to pretty-print generated bindings. Pre-existing, unrelated to
the pin, and the build exits 0 through it.

### 2. `mesa-26.2.1` exists — D21 is answerable now, and is still the owner's

`da14d65e4499e66468094be52bff9ea0915a695e`, tag object `0a4021394f88`, released
2026-08-20, [release notes](https://docs.mesa3d.org/relnotes/26.2.1.html).
This is the release [26.2.0's own notes](https://docs.mesa3d.org/relnotes/26.2.0.html)
told stability-minded users to wait for, and its absence is the reason every
previous entry gave for leaving D21 open. That reason is gone. **The decision is
not taken here**, because what makes it a decision has not changed:

- **On offer:** `VK_EXT_mesh_shader` on NVK, `VK_KHR_shader_fma`,
  `VK_EXT_shader_atomic_float`, `VK_NV_shader_atomic_float16_vector`,
  `VK_NVX_binary_import`, `VK_GOOGLE_display_timing` (WSI-shaped, and it would
  meet our backend), 159 nvk/nak/nil/nouveau commits; 26.2.1 itself adds "New
  features: None", 16 documented fixes, a Kepler *Failed to find free register*
  NAK panic fix, and the same `Serialize carry access in instr_sched_prepass`
  that 26.1.7 already brought here.
- **Cost, re-measured against the pin as it now stands** (`mesa-26.1.8` →
  `mesa-26.2.1`, not the 26.1.6 baseline the older rows used): **3569 files
  changed, 55 of the 117 we patch among them** — 21 `src/nouveau/compiler/`,
  8 `src/nouveau/vulkan/` (`nvk_wsi.c`, `nvk_physical_device.{c,h}`,
  `nvk_device.c`, `nvk_instance.c`, `nvk_cmd_buffer.c`, `nvk_cmd_draw.c`,
  `meson.build`), 4 `src/vulkan/wsi/`, both `vk_image.{c,h}`, 2 `src/nouveau/nil/`,
  2 `src/nouveau/headers/`, 9 `src/compiler/rust/`.

The two branches have no ancestry between them (`mesa-26.1.8` is not an ancestor
of `mesa-26.2.1`), the 49-patch series will not carry over mechanically, and
every hardware number in this file was measured against 26.1. The honest form of
taking it is a rebase branch with its own console run — not an edit to
`versions.env`. **D21 stays open, and it is now open on the merits rather than
on a missing tag.**
...
## Mesa 26.1.7, the first point release that touches files we patch (2026-08-12)

`mesa-26.1.7` was tagged on 2026-08-12, above the pinned `mesa-26.1.6`. It is a
point release *inside* the pinned series —
[notes](https://docs.mesa3d.org/relnotes/26.1.7.html): "New features: None",
89 commits from 34 authors closing 13 reported bugs, 121 files changed,
+8624/-587. **Taken.** The 26.2 series is still D21, still not taken, and the
26.2.1 its own notes tell stability users to wait for has still not been tagged
(`git ls-remote --tags … 'mesa-26.*'`, 2026-08-12).

### What is different about this one

26.1.6 was a clean miss: not one of the files it changed was among the 121
`mesa-patches/` writes to. 26.1.7 is the first point release where that
intersection is **not** empty — six files, computed rather than eyeballed:
...
Six overlapping files is a reason to read them, not a reason to stop:

| file | what 26.1.7 does to it | can it reach a Switch? |
|---|---|---|
| `meson.build` | `dep_clc` is now looked up only for rusticl / microsoft_clc, and prefers a `mesa-libclc` fork over `libclc` | **no** — `scripts/build-mesa-clc.sh` enables neither, so the native `mesa_clc` build stops needing libclc at all. A relaxation of our build, not a break |
| `src/util/os_misc.c` | a macOS `MAP_JIT` probe for processes without the JIT entitlement | **no**, all of it inside `DETECT_OS_APPLE` |
| `src/nouveau/compiler/nak/ir.rs` | adds `Dst::is_carry()` | only as the helper the next row uses |
| `src/nouveau/compiler/nak/opt_instr_sched_prepass.rs` | serialises carry-register access when building the scheduler's dependency graph | **yes** — no shader-model guard, so it is live on GM20B's SM50 |
| `src/nouveau/compiler/nak/sm70_encode.rs` | `OpFMnMx` stops encoding `RZ` as src2, and clears the SM90 `.IS_A` bit | **no**, SM70+ encoder; GM20B is SM50 and goes through `sm50_encode.rs` |
| `src/nouveau/vulkan/nvkmd/nouveau/nvkmd_nouveau_pdev.c` | Turing compression re-enabled against nouveau 1.4.3 | **no**, the nouveau KMD backend; ours is `nvkmd_horizon` |

So exactly one upstream change in this release can alter the code generated for
a Switch, and it is a correctness fix in the instruction scheduler — the class
of change worth taking rather than avoiding. The rest of the release is other
drivers: anv, radv, tu, freedreno, etnaviv, v3d, zink, d3d12, radeonsi, venus.

All 77 patches applied **unmodified — no fuzz, no rejects, and nothing in
`mesa-patches/` needed touching**, including the two that write to
`nvkmd_nouveau_pdev.c` (0029, 0038), the one that writes to all three NAK files
(0017) and the nine that write to the root `meson.build`.

Three files under `src/nouveau/compiler/` moved, which makes
`docs/rust-toolchain.md`'s blanket "nothing quoted here moved" claim from 26.1.6
untrue as written. It now says the precise thing instead: the three that moved
are not among the files that document cites, so its file:line evidence is still
exact — checked file by file rather than by area.

### What was actually run

| what | command | result |
|---|---|---|
| the pin resolves and verifies | `scripts/fetch-mesa.sh` | `checked out mesa-26.1.7, verified HEAD = e8617e4c…` |
...
against 26.1.5 or 26.1.6, and the NAK scheduler fix above is the only change
between them that can reach a shader this driver compiles.

---

## Run 31 — 91% of the compile, and Phase 7 closes (2026-08-11)

`t_vk_cache`, build `2026-08-11T18:58:20.516Z e6f6165-dirty mesa:3ba5227`,
...
**5342 us cold, 442 us warm. 12x faster, 91% of the compile gone.** On
the driver's own cache path, no `MESA_SHADER_CACHE_DIR`, and no
`VkPipelineCache` passed to the create — nothing in the process could
answer it but the disk. Beside it, unchanged from run 30: `4096/4096
words match`, the 64-word poisoned tail untouched, `hits = 2, misses = 0`.

### Why the 5342 can be believed

The cold run's own log was not kept, so this number arrives without a
file behind it. What stands in for one is a rule rather than a memory:
the test records a cold baseline **only** when the driver itself reported
`0 entries` at startup, and a cold run clears
`sdmc:/mesa_shader_cache` before the driver opens anything.

That rule is not theoretical here — it fired twice. Runs 29 and 29 both
intended a cold measurement, both started against a populated cache, and
both wrote `0` to the marker rather than record a warm create as cold. A
guard that has demonstrably refused the wrong number twice is what makes
the right number mean something the third time.

### What one shader does and does not say

It is one compute shader that multiplies and XORs. A real title has
hundreds of pipelines and much larger shaders, and the per-shader compile
grows with them while the cache read barely does. So 91% is a floor and
not a figure to quote — the useful claim is that the compile is what
disappears, which is what the two numbers show.

### Phase 7 is complete

All six exit criteria in `docs/milestones.md` are met, and four of them
on a console:

| | |
|---|---|
| the store against damaged files | **H**, `h_blob_cache` 145/145 under ASan/UBSan |
| no `mmap`/`flock`/`posix_fallocate`/`memfd_create`/`getpwuid_r`/`ftw` in the driver's archives | **X**, `nm -u` |
| `t_shader_cache` including a second launch | **HW**, run 28, PASS 56/56, 32 entries WARM |
| two builds must not read each other's entries | **HW**, runs 27 and 27 |
| a shader compiled once is not recompiled | **HW**, runs 29, 29 and 30, `hits = 2, misses = 0` |
| what that saves | **HW**, run 31, 5342 us against 442 us |

The pagaré `docs/history/phase-4.md:536` left open — *"if a later phase
enables the shader cache … `flock` comes back and needs an answer then"*
— is answered, and the answer ran.

---

## Run 30 — 4096 of 4096, and the measurement that keeps not happening (2026-08-11)

`t_vk_cache`, build `2026-08-11T18:46:12.351Z 28bbe35-dirty mesa:3ba5227`,
**`RESULT: PASS (44/44)`**. `docs/hw-logs/t_vk_cache-run30-PASS.log`.
...
A shader NVK did not compile, computing **every one of its 4096 words**
correctly, and writing nothing past its last invocation. Run 29 had the
same hits and misses but verified exactly one word, against a wrong
expectation; this is that test with its own two bugs fixed, and the
self-check on `expect_word(0)` passing is the guard against repeating
them.

`vkCreateComputePipelines` took **432 us** warm, against 646 us in run 29
— the same operation, and the difference is SD-card noise rather than a
change.

### The measurement that still has not happened

```
note the driver's own cache was already populated when this launch started
note no cold baseline on record, so the saving is not reported
```

Twice now the intended cold run has not been cold, because the driver's
cache survives from the previous run and deleting it was an instruction
printed in a log rather than something the test did. The guard added
after run 29 worked exactly as designed — it refused to record 432 us as
a cold baseline and wrote `0` to the marker instead, so no ratio was
derived from a number that was not what it claimed to be.

**A guard that keeps firing is a design problem, not a success.** The
cold path now clears `sdmc:/mesa_shader_cache` itself, before
`vkfw_init`, with `opendir`/`readdir`/`remove` on the calling thread and
only for `*.hzc`. What that costs is one recompile for anything else that
had shaders there, which is the entire downside of clearing a cache and
is why it is acceptable in a test whose subject is the cache.

The remaining instruction is one file: delete the marker, run twice.

### A number worth keeping for later

`vkfw_init` — instance, device, and the disk cache being opened — took
**295567 us**, and 293986 us in run 29. That is 0.3 s before a single
pipeline is created, it is stable across runs, and almost none of it can
be the cache: the file held two entries and opening a 201-entry store
measured 1605 us in run 28. It is not this phase's problem, but nothing
has ever measured what it *is*.

---

## Run 29 — NVK stops recompiling, and a test that did not know the answer (2026-08-11)

`t_vk_cache`, build `2026-08-11T18:37:03.904Z b5454be-dirty mesa:3ba5227`,
`RESULT: FAIL (37/39)`. `docs/hw-logs/t_vk_cache-run29-FAIL.log`.

### What it measured, which is the point of Phase 7
...
The driver's own cache directory, no `MESA_SHADER_CACHE_DIR`, no
`VkPipelineCache` passed to the create — so nothing in the process could
answer it but the disk. **Two hits, no misses: NVK compiled nothing.**

### Both failures were mine, and one of them is the expensive kind

```
FAIL the cached shader's output: 0/64 words match
note first mismatch at word 0: got 0xa5c4d00d, want 0x00000000
```

`comp_write_id` documents in its own header, and `t_vk_compute` computes,
`out[id] = (id * 2654435769) ^ 2781138957`. At id 0 the multiply vanishes
and the XOR constant is the whole answer: **0xa5c4d00d**. The console
returned precisely the right value and the test reported it as a cache
that had handed back a wrong shader.

That is the worst shape a test failure can take here — not a check that
could not fail, which is what run 27 had, but a check that fails *loudly
and wrongly* and points at the component under test. A day spent
believing it would have been spent looking for a corruption that does not
exist. The test now carries a static check that `expect_word(0)` is that
value, so a wrong expectation fails as a wrong expectation.

The second failure was the same line: the test dispatched **64 groups** of
a shader whose `OpExecutionMode` is `LocalSize 64 1 1` into a **64-word**
buffer — 4096 invocations, 4032 of them storing past the end of a runtime
array. It neither faulted nor hung, which is luck and not a result. The
geometry is now t_vk_compute's, which is the shader's, and there is a
64-word poisoned tail whose survival is checked.

### What run 29 does NOT establish

**That the shader off the card is correct in full.** One word is known
good. The other 4095 were compared against the wrong array and only the
first mismatch was printed, so they are simply unknown.

**The saving.** 646 us is a warm create with nothing to compare it to:
the run before it had already populated the driver's cache, so no cold
baseline exists.

### What changed so that one log is the whole measurement

The marker file now carries the cold `vkCreateComputePipelines` time
beside the build stamp, and a warm run prints both and the percentage
saved. That removes the need for whoever holds the console to keep two
logs side by side.

And the cold baseline is no longer taken on trust. The marker only knows
whether *this test* has run; it cannot know whether something else left
the same shader in `sdmc:/mesa_shader_cache`. So the test asks the driver,
which announces what it opened — `disk cache: <path>, N entries` — and
**refuses to record a cold time unless that said `0 entries`**, printing
the reason instead. A baseline that is quietly a warm number would make
every ratio derived from it wrong, which is the same failure as the check
that lied, one level up.

---

## Run 28 — a cache that outlives its process (2026-08-11)

`t_shader_cache`, build `2026-08-11T17:02:09.068Z 1f9f128-dirty
mesa:3ba5227` — the fixed binary. **`RESULT: PASS (56/56)`**.
`docs/hw-logs/t_shader_cache-run28-PASS.log`.

### The line the whole feature was waiting for
...
ok   C2 and the file did not have to be reset to do it
ok   C2 nor truncated — the previous launch's writes were whole
```

That is the claim a cache exists to make, and until this run nothing in
this project had shown it: entries written by one process, read by the
next, byte for byte, off a real SD card.

### The check that could not fail, now one that can

```
ok   A6 the reset file is readable (the file could be measured at all)
ok   A6 the store's own size is the size on the card
ok   A6 ftruncate() actually shortened the file on this filesystem
```

Three lines where run 27 had one, and the one it had was a `-1` compared
as a signed long. `ftruncate` through `fsFileSetSize` is evidenced now.

### The two defects, re-measured on the same console

| | run 27 | run 28 |
|---|---|---|
| open with 201 entries | 40498 us — **201 us/entry** | **1605 us — 7 us/entry** |
| 100 puts into a 32 KiB ceiling | **70 compactions**, 2575834 us | **5 compactions**, 637192 us |
| file size reported | `-1 bytes` | 25688 bytes, and 21688 after compaction |

**25x faster to open, 14x fewer rewrites.** The scan change was the
larger of the two and the more counter-intuitive: the 64 KiB stdio buffer
added to make the scan fast was what made it slow, because every `fseek`
that discards a buffer costs a full refill. Removing the seeks — read
forward, and only seek over a payload bigger than one refill — is what
took 201 us per entry to 7.

### One log line that is correct and reads like a fault

```
MESA: info: disk cache: …/mesa_cache/t_shader_cache.hzc did not match
this driver build or could not be read; started over (96 bytes discarded)
```

at **B1**, the first `disk_cache_create` of the run. 96 bytes is exactly
an empty file header for this driver key (a 30-byte blob padded to 32,
plus 64), i.e. the file the *previous* run's B7 left behind — B7 opens
under a deliberately different driver id and resets it. So the message is
the driver correctly refusing the previous run's leftovers.

Correct, and still worth removing: a log kept as evidence should not need
a footnote to be read. Section B now deletes its own cache file first, so
"did not match this driver build" appears only where the test provokes it.

### What is left, and it is the point of the exercise

**That NVK stops recompiling.** Everything measured so far is about the
store and about Mesa's API. No test has yet timed a shader compiled on
one launch and *not* recompiled on the next.

`tests/t_vk_cache.c` is new and does exactly that: one compute pipeline,
the driver's own cache directory with no environment overrides, timed —
and then **dispatched and its output verified**, because a cache handing
back a corrupt binary would create a pipeline just as happily and faster.
It needs two launches of the same build. It is built (36 `.nro` now) and
has never been launched.

A rebuild between the two runs empties the cache **by design**, since the
driver identity is a digest of `mesa-patches/` and `horizon/`. The test
records the build stamp in its marker file and reports a mismatch as
"cold" rather than as a cache that lost its contents.

---

## Run 27 — the cache on a console, and the check that could not fail (2026-08-11)

`t_shader_cache`, build `2026-08-11T00:07:01.401Z d0514c1-dirty
mesa:3ba5227` — the binary that was shipped, confirmed by the stamp in
the log's second line. `RESULT: FAIL (46/47)`. Log kept verbatim at
...
| | |
|---|---|
| `disk_cache_create()` returns a live cache on a Switch | B1, and the driver's own lines: `disk cache: …/t_shader_cache.hzc, 0 entries` → `2 entries`, `disk shader cache: hits = 2, misses = 1` |
| Mesa's API works end to end | B3 put → `wait_for_idle` → get with the bytes intact; **B4 a 256 KiB entry**, which permanently rules out the 64 KiB ceiling of the blob-callback path this design rejected |
| A file cut short mid-payload recovers on the real card | A7, all eight checks: the tear is noticed, four whole entries survive, the torn one is a miss and not a wrong answer, and the repaired file takes new writes |
| Another driver build's file is reset, not read | A6 and B7 — `did not match this driver build …; started over (624 bytes discarded)`. This is the defect found before shipping, now measured |
| An in-place compaction leaves a valid file | A8 reopens clean, no reset, no truncation, newest entry intact |

### The failure was mine, and the worse one passed

`file_size_of()` opened the cache file a second time while the store
still held it open. This platform refuses that — `STATUS.md` already
records it for the test framework's own log file — so the helper returned
−1, and two callers used it while the store was open:

- **A8** cast it to `uint64_t`: 2^64−1 > 32768, the single FAIL.
- **A6** compared it as a signed `long`: `-1 < 4096`, **true**. *"A6
  ftruncate() actually shortened the file on this filesystem"* is a check
  that could not fail, and it printed `ok`.

The FAIL is noise. The `ok` is the defect, and it is the same shape as
the one this file already records under *"The check that lied, and it was
mine"*: a check that cannot look must not answer. Callers now go through
`file_size_checked()`, which fails the test if the measurement did not
happen, and read `stats.file_size` while the store is open.

### Two structural defects the numbers exposed

| | run 27 | after the fix |
|---|---|---|
| open with 201 entries | **40498 us — 201 us/entry** | scan rewritten; to be re-measured on a console |
| 100 puts into a 32 KiB ceiling | **70 compactions, 2.58 s** | 200 puts → **11 compactions** (host) |

**201 us per record is one I/O per record**, to read a 56-byte header out
of a 50 KB file — not a sequential read. `bc_scan()` seeked to every
record, and whether a C library keeps its buffer across a seek is a
property of that library; newlib gates the optimisation on its own seek
function and on `fstat` describing a regular file with a block size, and
libnx's `fstat` sets `S_IFREG` and leaves `st_blksize` alone. Rather than
depend on how that resolves, the scan no longer seeks: it reads forward,
and steps over a payload by consuming it unless the payload exceeds one
buffer refill, where seeking moves fewer bytes. **The 64 KiB buffer added
to speed this up was probably what made it slow**, since every seek that
discards it costs a full refill.

**70 compactions for 100 puts** was arithmetic, not bad luck:
`bc_compact()` filled the file to `max_size`, so the next put crossed it
again. It now compacts to half the ceiling. Measured on host, 200 puts of
1 KiB into a 32 KiB cache: **11 compactions, 24 entries live, 25992
bytes** — against ~140 before. The host suite bounds it at 30 and that
bound **fails** if the watermark is put back to 100%, which was checked
rather than assumed.

### The cost that is not a defect

`fsync` per entry: 5292 us. It is on a background thread and compiling a
shader takes far longer, so the queue keeps up. Worth saying plainly that
it buys less than it looks — the format already recovers from an
unflushed tail by truncating it, which is exactly what a missing `fsync`
would cost. Changing it is a decision with a measurement attached and it
has not been taken.

Other numbers, for the record: put(4 KiB) 8148 us; get(4 KiB) 3408 us;
200 small puts 5292 us each; Mesa put + wait 4606 us; Mesa get 291 us;
Mesa put(256 KiB) + wait 5097 us.

### What run 27 did NOT reach

**Section C was cold**, as a first run must be: `this launch found 0
entries (COLD — first run on this console, which is a pass)`. So the
central claim of a cache — that entries outlive the process that wrote
them — has still never been shown. The file is on the card, so the next
launch starts warm.

And `ftruncate` is back on the unverified list, because the check that
said it worked could not fail.

---

## The shader disk cache, enabled (2026-08-11)

`docs/history/phase-4.md:536-541` left this as a pagaré:

> `getuid` and `flock` are still unresolved in the archives. They are
> unreachable *today*, with `-Dshader-cache=disabled` … If a later phase
> enables the shader cache — which `scripts/configure-mesa.sh` records as
> a decision, not a workaround — `flock` comes back and needs an answer
> then.

This is that answer. **Host and cross only. No console has run it.**

### What the limitation actually was

One Meson option in three scripts, and only two of them are about Horizon
(`scripts/build-mesa-clc.sh` is a native build where everything is off).
The reason recorded in `scripts/configure-mesa.sh:55-63` was accurate:
`src/util/disk_cache_os.c` is written against `mmap`, `flock`,
`fcntl(F_SETLK)`, `posix_fallocate`, `ftw` and `getpwuid_r`.

Measured again before touching anything, on devkitA64 gcc 15.2.0:

| | |
|---|---|
| `sys/mman.h` | **absent** — `fatal error: sys/mman.h: No such file or directory` at `disk_cache.c:34` |
| `ftw.h`, `sys/file.h`, `fcntl.h`, `dirent.h` | **present**, all four. The old note was wrong about these |
| `flock` | declared in `<sys/file.h>` and **not defined**: compiles, then `undefined reference to 'flock'` at link |
| `posix_fallocate`, `memfd_create` | same, undefined at link |
| zlib / zstd | **present**, 1.3.1 through devkitPro's portlibs. `HAVE_COMPRESSION` was already being defined |

That last row is worth naming: compression was never the problem, so
entries are deflated here exactly as they are everywhere else.

### The shape taken, and the one that was rejected

`disk_cache.c` — key derivation, the put queue, compression, the
statistics — **is upstream's and is reused**. It turned out to use none
of the five system headers it opens with; only the includes were in the
way. So the delta is the OS layer only.

Rejected, and recorded because it is the obvious idea: `disk_cache.c`
already has a storage-agnostic path through `disk_cache_set_callbacks()`,
which would have needed *no* change to Mesa at all. It caps entries at
64 KiB — `blob_get_compressed()`'s `max_blob_size`, inherited from
Android's `egl_cache_t` — and a pipeline binary above that would become a
permanent silent miss. `t_shader_cache` section B4 stores 256 KiB
specifically so that ceiling cannot be reintroduced unnoticed.

### The defect this work found before it shipped

**On Horizon the driver identity did not change when this port changed.**
Patch 0018 derives `driver_build_sha` from `PACKAGE_VERSION` and
`MESA_GIT_SHA1` because there is no ELF build id without a dynamic
loader. Both describe the pinned Mesa checkout. Neither moves when
`mesa-patches/` or `horizon/` move — and those hold NAK, NIL and the
whole platform layer.

Patch 0018 recorded that as an acceptable limitation, and while the value
was only `driverUUID` and the pipeline cache UUID it was one. It is not
acceptable once `nvk_physical_device.c:1349-1366` feeds the same value to
`disk_cache_create()` and `disk_cache_compute_key()` hashes it into every
key: it would mean the previous build's shader binaries served to this
build, with no error path and no line in any log. That is the same class
of bug as the reference cycle in 0058 and the check that lied in run 2,
and it would have been introduced *by* enabling the cache.

`scripts/gen-driver-id.sh` closes it — SHA-256 over `MESA_COMMIT`, every
patch in `mesa-patches/` and every `.c`/`.h` under `horizon/`, passed as
`-Dhorizon-driver-id`. Sources and not a timestamp, so a rebuild of
unchanged source keeps its warm cache and reproducible builds stay
reproducible. It moved when it should during this very session: `6f648660`
before the engine was reworked, `02fce8cb` after.

### Two dependencies of this repository that shaped the design

**libnx's path operations are not thread-safe.** `fsdev_fixpath()` copies
into the file-scope `__nx_dev_path_buf` with no lock, and every
path-based devoptab entry point goes through it
(`nx/source/runtime/devices/fs_dev.c:211`, and `char *fs_path =
__nx_dev_path_buf` at 405, 1111, 1197, 1225, 1276). `disk_cache.c` calls
the backend from its put queue, so a backend doing path work would be a
race whose symptom is a write to the wrong file. The first version of the
store compacted by writing a temporary file and calling `remove()` and
`rename()`; it now rewrites in place and performs **no path operation
after `open()`**.

**`UTIL_QUEUE_INIT_USE_MINIMUM_PRIORITY` is a no-op here.** It is
implemented `#if defined(__linux__)` (`u_queue.c:254`, `:334`), so
upstream's four "low priority" disk\$ threads would be four
normal-priority threads competing with a render loop on a console that
gives homebrew three cores. One thread on Horizon.

### What was run, and what it said

| what | command | result |
|---|---|---|
| host suites | `scripts/run-host-tests.sh` | PASS 20/20, 21/21, 30/30, 39/39, **16/16**, 8/8, **138/138** |
| layering | `scripts/check-layering.sh` | OK |
| absolute paths | `scripts/check-no-abs-paths.sh` | OK |
| Makefile ≡ meson | `scripts/check-mesa-test-parity.sh` | OK |
...
And the measurement the old comment was waiting for:
...
One real bug, in code written the same hour: `qsort()` on the `NULL` of
an empty snapshot in the new in-place compaction. UBSan caught it the
moment the case was covered. It is the argument for the store living in
`horizon/` rather than inside the Mesa patch, made concretely.

### What is NOT known

Everything a console decides. Whether `ftruncate` through `fsFileSetSize`
shortens a file on exFAT; whether an in-place compaction reopens clean
after a real power cut; whether an entry survives the process that wrote
it; what an `fsync` per entry costs inside a frame; whether the 64 KiB
stdio buffer chosen for the open scan was the right guess; whether two
builds of this port really refuse each other's entries. `t_shader_cache`
asks all of them and has never been launched.

There is also no measurement yet of the thing the feature exists for — a
shader compiled on one launch and *not* recompiled on the next — because
that needs `t_vk_*` timings across two runs on hardware.
...
## The 6.4 Hz report: what it is not, and the two things built for it (2026-08-11)

A Vulkan application presenting to the default window through `VK_NN_vi_surface`
reports **157 ms per frame** — 6.4 fps — at 1280x720 with three images on the
zero-copy path, handheld, under hbmenu title takeover. Its own instrumentation
puts 0.6 ms in command recording, 0.1 ms in `vkQueueSubmit` and 0.3 ms in
`vkQueuePresentKHR`. The report reads the stability of that total as an external
pacer releasing buffers at 6.4 Hz.

**Nothing has been fixed and nothing has been reproduced.** What follows is one
argument from the reporter's own numbers, one gap in this project's evidence, and
two pieces of code built to close it. Every number below that is not the
reporter's own is a cross build.

### The four measurements do not describe a pacer

| configuration | frame total |
|---|---|
| IMMEDIATE, wait in `vkAcquireNextImageKHR` | 157 ms |
| FIFO, wait in the first draw | 157 ms |
| `MESA_VK_WSI_HORIZON_FORCE_COPY=1` | **183 ms** |

The fourth row is the one that decides it. Under an independent 157 ms pacer the
copy fallback's extra work — a row copy plus libnx's block-linear swizzle — lands
inside a wait the loop was going to spend anyway, and the frame total does not
move: `wsi_horizon_acquire_fallback` returns immediately and the block happens in
`framebufferBegin`, so the release rate, not the loop, sets the period. The total
moved by 26 ms. **That is a serial link in a dependency chain, not a clock.**

Both paths were configured with the same queue depth, so the comparison is
apples to apples: `wsi_horizon_init_fallback` creates its Framebuffer with
`MIN2(image_count, WSI_HORIZON_MAX_FB_IMAGES)`, i.e. three.

The same 26 ms also refutes the report's third hypothesis — that something walks
the scanout buffer with the CPU on uncached memory at ~25 MB/s. That 26 ms *is* a
full CPU pass over a buffer of exactly that size (3932160 B), i.e. about
150 MB/s, and it includes the swizzle, so the real traffic is roughly double. And
on the zero-copy path nothing touches the buffer with the CPU at all: `memcpy`
and `wsi_image::cpu_map` appear in `wsi_horizon.c` only inside
`wsi_horizon_present_fallback`.

### The gap in this project's evidence, and it is ours

These two sets were **disjoint** until today:

```
tests that draw    : t_vk_caps t_vk_depth t_vk_format t_vk_texture t_vk_triangle
tests that present : t_vk_immediate t_vk_suboptimal t_vk_swapchain t_vk_wsi_mt
```

Every presenting test's frame is a `vkCmdClearColorImage` — `t_vk_swapchain.c:436`,
`t_vk_immediate.c:535`, `t_vk_wsi_mt.c:630`, `t_vk_suboptimal.c:398` — and the
pattern frames are a buffer-to-image copy. `t_vk_triangle` is 64x64 off-screen. So
**no test in this tree has ever driven the 3D pipeline's ROP into a scanout
surface**, and "three images present 90 of 90 at a mean of exactly 16666 us"
covers clears and copies only. "3 images vs 2" as a candidate is separately
already closed by run 16, which measured three.

### What was built

**`tests/t_vk_present_draw.c`** — a presented frame a graphics pipeline drew: one
fullscreen textured triangle sampling a surface-sized RGBA8 texture, the shape of
a guest-output blit. Sections B (submit-to-fence cost on the scanout image
itself, clear / one draw / three draws), C (pacing, pipelined, clear then draw
then clear), D (three draws under IMMEDIATE — the reporter's exact
configuration), E (the relation). E is the check the file exists for:

> a presented interval must be no larger than `max(refresh, GPU time)` plus one
> refresh of slack

A console where the GPU costs 1 ms and frames arrive 157 ms apart **fails** it and
names the presentation engine; a console where the GPU costs 145 ms **passes** it
and section B is then the finding. Neither is assumed. The clear runs are asserted
to be paced at the refresh *first*, because a run where the control is not paced
is not interpretable. Every run is capped at four seconds as well as 180 frames —
at 157 ms a frame, 180 frames is 28 seconds and six runs is three minutes.

**`mesa-patches/0076`** — `MESA_VK_WSI_HORIZON_ACQUIRE_STATS=1`, one line a second
splitting the zero-copy acquire's wait into the two things it can be: the dequeue
loop (the consumer has not handed a buffer back) and `nvMultiFenceWait` on the
release fence (it did, and had not finished reading it). From outside the driver
those are one number. Off by default, two adds per acquire when off.

### What was actually run

| what | result |
|---|---|
| `horizon_ninja -C build/mesa-nvk src/vulkan/wsi/libvulkan_wsi.a` | compiles and links clean |
| `horizon_ninja -C build/meson` | **89/89 targets, 34 `.nro`**, including `t_vk_present_draw.nro` |
| `check-{layering,no-abs-paths,mesa-test-parity,history-intact}.sh` | all OK |
| `check-dispatch-complete.sh` | OK (825 entry points named, 234 core, 1 allowed absence) |
| `check-tls-relocs.sh` | OK |
| `scripts/run-host-tests.sh` | **not run** — no host `cc` on this machine. Neither change touches `horizon/` or `compat/`, which is what those suites cover |

**No console has run either of them.** The 157 ms and 139.6 ms are the reporter's
application-side numbers, not this test's output.

### One thing found on the way, and it is about this working tree

`scripts/apply-mesa-patches.sh --list` refuses here: `mesa/` is a shallow checkout
whose base is `6a02618 VERSION: bump for 26.1.5`, while `toolchain/versions.env`
pins `MESA_COMMIT=ffa422e5…` (26.1.6). The entry below records 26.1.6 being taken
and verified; **this working copy was never re-fetched**, so it is still building
26.1.5 (`PACKAGE_VERSION="26.1.5"` in `build/mesa-nvk/build.ninja`). That does not
affect patch 0076 — it touches only `wsi_horizon.c`, which patch 0053 adds and
which the 26.1.5→26.1.6 delta does not contain — but `scripts/fetch-mesa.sh`
followed by `scripts/apply-mesa-patches.sh` is owed before the next release build.

---

## Mesa 26.1.6, and the 26.2 series left as a decision (2026-08-10)

Checking upstream for releases newer than the pinned `mesa-26.1.5` found
two, and they are not the same kind of thing:

| | released | what it is |
|---|---|---|
| `mesa-26.1.6` | 2026-07-29 | a point release **inside** the pinned series. [Notes](https://docs.mesa3d.org/relnotes/26.1.6.html): "New features: None", 101 commits from 42 authors closing 15 reported bugs |
| `mesa-26.2.0` | 2026-08-05 | a new development series. [Notes](https://docs.mesa3d.org/relnotes/26.2.0.html): 78 new features, and its own first paragraph says people who care about stability should wait for 26.2.1 |

**26.1.6 was taken. 26.2 was not**, and the reason is the same number in
both cases: how much of this port the release moves underneath.

### What 26.1.6 changes, and why it was safe to take

101 commits, 114 files, +9551/-475. **None of those files is one of the 121 `mesa-patches/` writes
to** — the intersection is empty, computed rather than eyeballed:

```
grep -h '^+++ b/' mesa-patches/*.patch | sed 's|^+++ b/||' | sort -u   # 121
git -C mesa diff --name-only mesa-26.1.5^{} mesa-26.1.6^{} | sort -u   # 114
comm -12 <those two>                                                   # 0
```

Two of the 114 are NVK, and both are worth having:

- `nvk: free copy_memory_indirect_temps on command buffer destroy` — a
  `util_dynarray_fini` missing from `nvk_destroy_cmd_buffer`, i.e. a leak
  on the command-buffer destroy path. This tree has spent real time on
  exactly that class of bug (the 0058 reference cycle), and `t_log_scan`
  is the check that would eventually catch it.
- `nvk: report fills from memory correctly` —
  `VK_KHR_pipeline_executable_properties` reported `num_spills_to_mem`
  under the "Fills from memory" statistic. Cosmetic here; nothing in this
  tree reads it.

Nothing under `src/nouveau/compiler/` or `src/compiler/rust/` moved, which
is why `docs/rust-toolchain.md`'s file:line evidence is still exact and
says so now instead of quietly aging.

Everything else in the release is other drivers: anv, radv, tu, panvk,
v3d, d3d12, radeonsi, and the video runtime.

### What was actually run

| what | command | result |
|---|---|---|
| the pin resolves and verifies | `scripts/fetch-mesa.sh` | `checked out mesa-26.1.6, verified HEAD = ffa422e5...` |
| the series still applies | `scripts/apply-mesa-patches.sh` | **75 of 75, no fuzz, no rejects** |
...
### Why 26.2 is D21 and not a second commit

3588 files changed between 26.1.6 and 26.2.0, and **53 of the 121 this
project patches are among them**: 20 NAK files, 8 under
`src/nouveau/vulkan/` (`nvk_wsi.c`, `nvk_physical_device.{c,h}`,
`nvk_device.c`, `nvk_instance.c`, `nvk_cmd_draw.c`, `meson.build`,
`nvkmd/nouveau/nvkmd_nouveau_pdev.c`), 4 in `src/vulkan/wsi/`, both
`src/vulkan/runtime/vk_image.{c,h}`, `meson.build` and `meson.options` at
the root, and the Rust helper crates NAK is built on — which 26.2
refactored hard (`as_slice`, `smallvec`, and the proc macro NAK stopped
using).

The offer is real: `VK_EXT_mesh_shader` on NVK, `VK_KHR_shader_fma`,
`VK_EXT_shader_atomic_float`, `VK_NV_shader_atomic_float16_vector`,
`VK_NVX_binary_import`, `VK_GOOGLE_display_timing` — that last one being
WSI-shaped, so it would land on the backend this repository just spent a
phase building — and 159 nvk/nak/nil/nouveau commits, among them a zcull
crash fix and a UBO bounds-check that was signed and should not have been.

But the series does not carry over mechanically, and the port's whole
evidence base is hardware runs against 26.1. The honest form of that move
is a rebase branch with its own console run, which is a decision for the
owner: **D21**.
...
## `VK_SUBOPTIMAL_KHR`, and the result it was hiding behind (2026-08-10)

**Class: hardware (HW), and read the boundary.** Patch **0074** and a
new test, `t_vk_suboptimal`, **`RESULT: PASS (273/273)`** on run 21
(`docs/hw-logs/t_vk_suboptimal-run21-PASS.log`). But the condition the
patch exists for **cannot be provoked from inside the process**, so
`VK_SUBOPTIMAL_KHR` itself has still never been observed on a console —
what run 21 establishes is the rule around it and the absence of false
positives. See "What run 21 did and did not settle" below.

### What was wrong

This backend never returned `VK_SUBOPTIMAL_KHR` at all. A display mode
change — docking the console, which takes the layer to 1920x1080 — was
reported as `VK_ERROR_OUT_OF_DATE_KHR` at the next acquire, and that
result **promises something this backend does not do**: the
specification's words are "further presentation requests using the
...
over; `bqQueueBuffer` validates the crop against the *buffer* rather
than against the layer, so it keeps succeeding; and the layer's scaling
mode is what puts the smaller image on the larger display. The frames
still arrive, scaled rather than native — which is the definition of
`VK_SUBOPTIMAL_KHR` and not of the result that was being returned.
...
swapchain; one that ignores the result — legal for `SUBOPTIMAL`, not for
`OUT_OF_DATE` — stalled outright, because the acquire refused to hand
over an image it could have handed over.

### And the recovery was a loop with no exit, on the default path

Found while writing the test, and it is the more serious half.

`NWindow::default_*` is where libnx keeps the consumer's answer, and it
maintains it in `_nwindowUpdate` from every `nwindowQueueBuffer`. The
zero-copy path drives the producer directly through `bqQueueBuffer` —
deliberately, for the reasons in `wsi_horizon.c`'s opening comment — so
**libnx never saw the queue's output** and that field stayed at whatever
`bqConnect` reported at start-up, forever.

`wsi_horizon_get_extent` reads exactly that field. So
`VkSurfaceCapabilitiesKHR::currentExtent` would have kept reporting the
pre-dock size: an application told its swapchain was suboptimal would
have asked the surface what to build instead, been handed the size it
already had, and built the same suboptimal swapchain again. The copy
fallback never had it, because `framebufferEnd` goes through
`nwindowQueueBuffer` and libnx updates the field itself.

0074 writes the two `u32`s where libnx writes them, from the queue
output the zero-copy present already reads.

**Read out of the compiled library, not assumed.** `bqQueueBuffer`'s
signature settles one direction by itself — `Result bqQueueBuffer(Binder
*b, s32 buf, const BqBufferInput *input, BqBufferOutput *output)`
(`buffer_producer.h:44`) takes a `Binder` and no `NWindow`, so it
structurally cannot maintain `NWindow` fields. The other direction is in
`native_window.o`, disassembled out of the image's `libnx.a`: after
`bl bqQueueBuffer`, `nwindowQueueBuffer` stores `-1` to `[x19, #64]`,
copies eight bytes of the `QueueBufferOutput` from the stack to
`[x19, #112]` as one `d`-register store, and sets `[x19, #126]` from
`numPendingBuffers > 1`. Against the `NWindow` layout in
`native_window.h` those offsets are `cur_slot`, the
`default_width`/`default_height` pair, and `consumer_running_behind`. So
libnx maintains exactly the two fields 0074 writes, in exactly the place
it writes them, and only on the path this backend does not take.

### The line, stated once and in one place

`docs/wsi.md` § 5.1 now carries the full table, and the same list is in
the comment above `wsi_horizon_extent_changed()` so the two cannot
drift. In short: **exactly one condition is suboptimal** — the VI
consumer reporting a layer size other than the one the swapchain
registered its buffers at — and every other refusal this backend makes
is `VK_ERROR_OUT_OF_DATE_KHR`, because in each of those the next present
really cannot happen: another swapchain has taken the window, the queue
answered something that is not a retry, the compositor handed back a
slot we already hold, the fallback has no framebuffer, or the
application holds every image and is blocking for one only it can free.

Two details that are easy to get wrong and are stated in the code:

- **`VK_SUBOPTIMAL_KHR` is a success code**, so the check runs *after*
  the acquire and only when an image came back. `wsi_common_acquire_
  next_image2` marks the image acquired and signals the caller's
  semaphore and fence on it (`wsi_common.c:1966`), so returning it in
  place of an acquire would hand the application an index it never got.
  `VK_TIMEOUT`, `VK_NOT_READY` and every error go back untouched.
- **No sticky flag.** It is a comparison of two current values, so it
  answers "suboptimal now": true while the console stays docked, which
  is what repeats the result until the application recreates, and false
  again by itself if the console is undocked, which is correct.

### What the test can and cannot do, and it says so in its own log

`tests/t_vk_suboptimal.c`. The oracle is the surface, not a flag in the
driver: every check is written against
`VkSurfaceCapabilitiesKHR::currentExtent`, which is the property the
specification's definition of `VK_SUBOPTIMAL_KHR` refers to. The
invariant asserted per frame, in **both** directions, is

> `SUBOPTIMAL` ⟺ `currentExtent` ≠ the swapchain's `imageExtent`

which is what makes "section A saw no `SUBOPTIMAL`" mean something: it
saw none *and* the surface it was measured against matched, so the
absence is a result rather than a check that cannot fire. A driver that
returned the code on a whim fails the forward direction; one that never
returns it fails the reverse the moment the console is docked.

| | |
|---|---|
| A | normal operation on whichever path the driver picks, 120 frames, every acquire and present exactly `VK_SUCCESS` |
| B | the same on the forced copy fallback, because the two paths learn the consumer's size through different calls |
| C | the results stay distinct: a superseded swapchain is `OUT_OF_DATE` from both acquire and present *while the surface's extent is unchanged*, so that result cannot be coming from the resize check; and an acquire that cannot be satisfied is `VK_TIMEOUT`, not a success code. Both paths |
| D | the mode change itself, with the operator, bounded and skippable |
| E | eight recreation generations alternating present path, then a swapchain created after every one of them was destroyed — a retained slot or an unreleased window shows up as a later generation failing to acquire — then the log read back for anything the driver said it could not tear down |

**Section D is the one that matters and the one that has not run.** The
producer side of a BufferQueue can read the consumer's default buffer
size and cannot set it: it belongs to VI. `viSetLayerSize` is a manager
service a homebrew does not have, and libnx keeps the default window's
`ViLayer` private in any case. So no sequence of calls available to this
process resizes the layer, and **nothing was invented to pretend
otherwise** — no debug hook, no injected extent, no environment
variable that makes the driver act as if the window had changed. The
section asks the operator, waits up to 30 s while presenting, and on a
run where nobody docks writes

> `D: no mode change happened (…), so SUBOPTIMAL was never provoked and
> THIS COVERAGE DID NOT RUN.`

rather than emitting a check that passes.

### What run 21 did and did not settle

Build `2026-08-10T13:44:43.834Z 13336c0-dirty mesa:c5e9e66`, game mode
(3155 MiB), **PASS 273/273**, 0 FAIL.

**Read the pass carefully.** Sections A, B, C and E would pass on the
*old* driver too: with the console undocked and nothing resizing the
layer, a backend that never returns `VK_SUBOPTIMAL_KHR` and one that
returns it only on a resize are indistinguishable. What run 21 settles
is the other half — the half a wrong implementation fails:

| | |
|---|---|
| **2303 frames with the rule checked in both directions**, both present paths, **0 disagreements** | `SUBOPTIMAL ⟺ currentExtent ≠ imageExtent`, per frame, against the surface rather than a driver flag. Section D alone is **1803 frames** — 30 s of continuous presenting with acquire and present each asserted every frame. This is the "no false SUBOPTIMAL" measurement |
| the two results stay distinct | a superseded swapchain answers `OUT_OF_DATE` from acquire *and* present **while the surface's extent is checked unchanged**, so it cannot be coming from the resize check; an unsatisfiable acquire answers `VK_TIMEOUT` after 3 images held. Both paths |
| recreation and nothing retained | 8 generations alternating present path, 20 of 20 frames each, then a swapchain created after all of them were destroyed; `no slot was left with the compositor`, `the driver tore down every object it created` |

**What it does not settle: `VK_SUBOPTIMAL_KHR` has never been returned
on hardware.** Nobody docked the console, and nothing in the process
can. The log says so itself rather than passing quietly:

> `D: no mode change happened (the wait expired after 1803 frames), so
> SUBOPTIMAL was never provoked and THIS COVERAGE DID NOT RUN.`

A run in which somebody docks the console during section D is what
closes it, and it is the one thing still outstanding for this patch.

**The binary was checked, not assumed** — run 11 is what a stale driver
looks like in this project. Both `wsi_horizon_swapchain_queue_present`
and `wsi_horizon_swapchain_acquire_next_image` in the shipped
`t_vk_suboptimal.elf` build `0x3B9ACDEB` (1000001003) into `w0` with a
`mov`/`movk` pair and return it. The stamp names `mesa:c5e9e66` and
`git -C mesa log` does not contain that hash: it is 0074's commit before
this run's evidence was written into its message, and amending a message
changes the hash and nothing else — the same relationship 0071 records
between `mesa:597ea0a` and `mesa:85638f8` for run 20. Stated here rather
than left for a reader to trip over, because an artefact naming a commit
that is not in the history is exactly what D20 was about.

**Two side results, both first-time evidence for earlier patches.**
Run 20's log carries **188** `vk_log*() called with client-invisible
object` warnings, which patch **0073** exists to take to zero; run 21 is
the first console run since and it is **zero**. And patch **0072** is
exercised by section C's copy-path recreation and four of section E's
generations without the console or `qlaunch` going down — the same
sequence that produced run 18's system fatal.

### Also in this change: three more Windows defects in our own tooling

Found by running the build gates on this machine, all the same class as
the three the PR 9 review found, and all of them blocking:

- **`fetch-rust-crates.sh` could not satisfy its own checksum gate.**
  Its Python heredoc wrote `packages.txt` in text mode, so on Windows
  the file is CRLF and the shell's `read -r` leaves the `\r` on the
  checksum. Every crate then "did not match the lockfile checksum" and
  failed with the two hashes printed *identically*:
  `addr2line-0.27.0.crate hashes to efe1709…, but the lockfile says
  efe1709…`. `newline='\n'`, as `split-status.py` already does.
- **`build-toolchain-image.sh`'s two verification steps lacked
  `MSYS_NO_PATHCONV=1`**, which `horizon_run` has carried since it was
  written. MSYS rewrote `-w /c/Users/…` into the drive-letter spelling
  and the daemon refused it — after a fifteen-minute image build, on the
  step whose whole job is to prove the image works.
- **Every Meson build directory in `build/` was configured against a
  container mount path that no longer exists** (`/run/desktop/mnt/host/c/…`
  against today's `/c/…`), so ninja regenerated on every invocation and
  the regeneration could not run. `build/meson`, `build/mesa-nvk` and
  `build/mesa-clc` were removed and reconfigured; nothing tracked was
...
| D2 | Mesa version to pin | **closed at Phase 2 start: `mesa-26.1.5`** @ `6a02618ccf6c5651ecb9cccbde571eb61fd73592`. **The pin is now `mesa-26.1.8`** @ `0fadfea4f394211946f308458f614839ef253ee8` (2026-08-31), by way of `mesa-26.1.7` @ `e8617e4ca95fc655b0f13fd115c224d27eba2441` (2026-08-12) and `mesa-26.1.6` @ `ffa422e53d59a4938b38abd5c3fc319555da31dd` (2026-08-10) — point releases *inside* the series this row chose. 26.1.6 cost nothing measurable (no file we patch was touched); 26.1.7 touches six of them and still applies unmodified, 77 of 77, with one change — NAK's carry-access serialisation — that can reach a Switch shader; 26.1.8 is a clean miss again, has no `src/nouveau/` commit at all, and leaves the series' net diff byte-identical. That is not a reopening of D2; **which series to be on is D21** |
...
| D21 | Move the pin off the 26.1 series onto Mesa 26.2 | **open, raised with the owner (2026-08-10), and on 2026-08-31 it is finally answerable: `mesa-26.2.1` EXISTS** — `da14d65e4499e66468094be52bff9ea0915a695e`, tag object `0a4021394f88`, released 2026-08-20, [notes](https://docs.mesa3d.org/relnotes/26.2.1.html) — which is the release 26.2.0's own notes told stability users to wait for. That was the *only* thing blocking the decision; the decision itself is still the owner's and is still not taken here. **What is on offer** (from [26.2.0's notes](https://docs.mesa3d.org/relnotes/26.2.0.html), `mesa-26.2.0` @ `9f0a761020bca92f2b07156a0621e5360cb8eca5`, 2026-08-05): `VK_EXT_mesh_shader` on NVK, `VK_KHR_shader_fma`, `VK_EXT_shader_atomic_float`, `VK_NV_shader_atomic_float16_vector`, `VK_NVX_binary_import`, `VK_GOOGLE_display_timing` (a WSI-shaped feature that would meet our backend), and 159 nvk/nak/nil/nouveau commits including a zcull crash fix and a UBO bounds-check signedness fix; 26.2.1 adds "New features: None", 16 documented fixes, and a Kepler *Failed to find free register* NAK panic fix. **What it costs, re-measured against the pin as it now stands** (`mesa-26.1.8` → `mesa-26.2.1`, not the 26.1.6 baseline the previous version of this row used): 3569 files changed, **55 of the 117 files `mesa-patches/` modifies among them** — 21 under `src/nouveau/compiler/`, 8 under `src/nouveau/vulkan/` (`nvk_wsi.c`, `nvk_physical_device.{c,h}`, `nvk_device.c`, `nvk_instance.c`, `nvk_cmd_buffer.c`, `nvk_cmd_draw.c`, `meson.build`), 4 in `src/vulkan/wsi/` (`wsi_common.{c,h}`, `wsi_common_private.h`, `meson.build`), both `src/vulkan/runtime/vk_image.{c,h}`, 2 in `src/nouveau/nil/`, 2 in `src/nouveau/headers/`, and 9 of the Rust helper crates NAK sits on under `src/compiler/rust/`, which 26.2 refactored hard. So the 49-patch series does **not** carry over mechanically, and every hardware number in this file was measured against 26.1. Not decided here: a series move is a decision under R14/D2, and the honest form of it is a rebase branch with its own console run, not a one-line edit to `versions.env` |
