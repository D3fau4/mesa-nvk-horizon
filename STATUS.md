...
**Evidence class X — cross build only. No console has run it.**
`scripts/ci-build-archives.sh` exits 0 in the toolchain image over devkitA64
against Mesa 26.1.6 + the now-78-patch series. All four files compile into
`libnvk.a` with no warnings — `[862/878] … nvkmd_horizon_dev.c.o`, and
`[867/878] _defer.c.o`, `[870/878] _ctx.c.o`, `[873/878] _va.c.o` — 35 `.nro`
link, `check-dispatch-complete` reports OK (825 entry points, 234 core, 1 allowed
absence) and `check-tls-relocs` OK (3 objects use TLS, all with relocations, 350
scanned). `scripts/apply-mesa-patches.sh --list`: **78 applied, 0 pending**.
The six host suites pass 132/132 and the four fast gates are OK — a
no-regression gate, not coverage of this code, which lives in Mesa and cannot be
reached from `run-host-tests.sh` by design. **That is the whole of it: it
compiles and links. Nothing here says it works.**
