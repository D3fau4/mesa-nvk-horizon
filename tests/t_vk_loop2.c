/*
 * comp_loop with the exact bound idiom of Godot's cluster loops (packed
 * min/max word, prmt-style extraction, bcsel, loop from item_from). Otherwise
 * the same as t_vk_loop. A loop whose trip count comes from memory, in the shape the Godot 4.1
...
#include "comp_loop2.spv.h"

const char *const test_name = "t_vk_loop2";
...
{
   uint32_t n = 0, i = 0;
   const uint32_t word = buf[OFF_TO + id];
   const uint32_t item_min = word & 0xffffu, item_max = word >> 16;
   const uint32_t from = item_min >> 5;
   const uint32_t to = item_max == 0 ? 0 : ((item_max - 1u) >> 5) + 1u;
   if (buf[OFF_ACT + id] != 0) {
      for (i = from; i < to; i++) {
...
static void fill_inputs(uint32_t *buf)
{
   for (uint32_t id = 0; id < LANES; id++) {
      ...
      uint32_t to = 0, act = 1;
      switch (g) {
      /* to is the packed cluster word: item_min | item_max << 16 */
      case 0: to = 0; act = 1; break;
      case 1: to = 0; act = lane < 8; break;
      case 2: to = ((lane % 4) * 40u) << 16 | (lane % 3) * 33u; act = 1; break;
      default: to = ((lane % 5) * 24u) << 16 | (lane % 2) * 32u; act = lane < 12; break;
      }
      buf[OFF_TO + id] = to;
      buf[OFF_ACT + id] = act;
      buf[OFF_N + id] = POISON;
