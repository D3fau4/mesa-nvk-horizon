/*
 * Nested control flow deep enough to need a backed convergence stack,
 * wrapped around one loop. sm50's crs_size() reserves no shader local
 * memory for the convergence stack below a depth of 16, counting one
 * stack slot per nesting level; nouveau counts two for the same field
 * ("factor 2 only needed where joinat/precont is used", nvc0_program.c)
 * and its compute path just reserves 0x800 bytes unconditionally
 * (nve4_compute.c). A shader between those two rules runs on the
 ...
 * shape with nothing else around it.
 *
 * Two warps of 32 invocations. Warp 0 goes all the way in and loops;
 * warp 1 stops at the outermost level, which is the case that works even
 * when the stack is not backed, so a failure says which one broke. The
 * CPU recomputes every output word.
 *
...
/* Generated from tests/shaders/comp_crs.spvasm by scripts/spv-embed.py. */
#include "comp_crs.spv.h"

const char *const test_name = "t_vk_crs";
const bool test_uses_display = false;

#define LOCAL_SIZE_X 32u
#define GROUPS       2u
#define LANES        (LOCAL_SIZE_X * GROUPS)   /* 64 */
#define LEVELS       12u
...
{
   uint32_t n = 0, i = 0;
   if ((buf[OFF_COND + id] & ALL_LEVELS) == ALL_LEVELS) {
      const uint32_t to = buf[OFF_TO + id];
      for (i = 0; i < to; i++)
         n += i + 1u;
   }
   *n_out = n;
...
/* Warp 0 reaches the loop; warp 1 turns back at the outermost level. */
static void fill_inputs(uint32_t *buf)
{
   for (uint32_t id = 0; id < LANES; id++) {
      const uint32_t g = id / LOCAL_SIZE_X, lane = id % LOCAL_SIZE_X;
      buf[OFF_COND + id] = g == 0 ? ALL_LEVELS : 0u;
      buf[OFF_TO + id] = 1u + lane % 4u;
      buf[OFF_N + id] = POISON;
      buf[OFF_I + id] = POISON;
   }
}
...
int run_test(test_ctx *t)
{
   ...
   /* --- what the GPU computed, one check line per pattern ---------- */
   {
      ...
      static const char *const pattern_name[GROUPS] = {
         "all 12 levels entered, then the loop",
         "turned back at the outermost level",
      };
