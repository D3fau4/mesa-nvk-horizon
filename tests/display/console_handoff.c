...
TEST_CASE_DECL(display, console_handoff)
{
    ...
    * version and it asserted a constant the build had already fixed —
    * it could not fail, and it did not test the thing its own message
    * claimed. Found in review of PR #7. (The constant now lives in
    * tests/display/suite.c, which changes nothing about the argument:
    * reading a definition back is not a measurement wherever it is.)
    *
   ...
   t_check(t, con != NULL && !con->consoleInitialised,
           "no console was started: main() acted on the suite\'s "
           "test_uses_display rather than merely being told about it");
