...
| Vulkan, through NVK | 19 | the full NVK driver built | **the Meson path only** |
...
2. Launch them from hbmenu **in this order** (later tests assume the
   ...
   | 27b | `t_vk_pipelines` | 96 distinct compute pipelines back to back — the only thing that makes the contiguous shader heap grow past the chunk it binds at device creation — plus create/destroy churn over reused heap addresses, and the per-pipeline compile times an application feels as stutter |
