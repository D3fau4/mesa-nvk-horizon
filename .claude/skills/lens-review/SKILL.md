...
- **lenses** (optional) — one or more lens codes or names, however the caller expresses them: a spoken request, or a directive of the form `skill:lens-review lenses=<code>[,<code>...]`. Default: every applicable lens (a full review).
...
- `{workflow.<name>}` resolves to fields in `customize.toml`'s `[workflow]` table (overrides win per the merge rules documented there).
